...
import show from "./assigment-assest/hide-password.svg"
import login_bg_1 from "./assigment-assest/login-bg-1.svg"
import logo from "./assigment-assest/Logo.svg"
import logo2 from "./assigment-assest/smallScreen.png"
  ...
  const navigate=useNavigate()
    ...
    const handlesubmit=async(e)=>{
        ...
        if(user.email!=="" && user.password!==""){
          let res=await axios.post("https://techprimes.onrender.com/login",user)
          console.log(res.data)
          if(res.data.msg==="Invalid User")
          {
            setinvalid(true)
          }
          else if(res.data.msg==="Success Full Registration"){
            setinvalid(false)
            login(res.data.msg)
